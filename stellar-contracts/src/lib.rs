...
impl PetChainContract {
    ...
    */
    ...
    pub fn get_weight_trend(env: Env, pet_id: u64, last_n: u32) -> Vec<WeightEntry> {
        if env
            .storage()
            .instance()
            .get::<DataKey, Pet>(&DataKey::Pet(pet_id))
        ...
        let start = if last_n as u64 >= count {
            1u64
        } else {
            count - last_n as u64 + 1
        };
        ...
        for i in (start..=count).rev() {
                ...
                .get::<NutritionKey, u64>(&NutritionKey::PetWeightByIndex((pet_id, i)))
            {
                if let Some(entry) = env
                    .storage()
                    .instance()
                    .get::<NutritionKey, WeightEntry>(&NutritionKey::WeightEntry(wid))
                {
    ...
    pub fn get_active_transfer_proposals(env: Env, pet_id: u64) -> Vec<PetTransferProposal> {
        ...
        for i in 0..active_proposal_ids.len() {
            if let Some(proposal_id) = active_proposal_ids.get(i) {
                if let Some(proposal) = env
                    .storage()
                    .instance()
