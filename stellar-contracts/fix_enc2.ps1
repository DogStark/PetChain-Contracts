...
$startIdx = 204284
$endIdx   = 208587  # one past closing brace of decrypt_sensitive_data
...
fn sha256_block(env: &Env, key: &Bytes, nonce: &Bytes, block_idx: u64) -> BytesN<32> {
...
fn encrypt_sensitive_data(env: &Env, data: &Bytes, key: &Bytes) -> (Bytes, Bytes) {
    ...
    while offset < data_len {
        let ks_block: BytesN<32> = sha256_block(env, key, &nonce, block_idx);
        let block_bytes: [u8; 32] = ks_block.into();
    ...
    let tag_bn: BytesN<32> = env.crypto().sha256(&tag_preimage);
    let tag: Bytes = tag_bn.into();
...
fn decrypt_sensitive_data(
    ...
    let expected_bn: BytesN<32> = env.crypto().sha256(&tag_preimage);
    let expected_tag: Bytes = expected_bn.into();
